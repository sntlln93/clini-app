...
async function readTransitionDurations(page: Page) {
    ...
    const raw = await button.evaluate(
...
test.describe('prefers-reduced-motion: reduce', () => {
    ...
    test('collapses an infinite animation to a single iteration', async ({
        page,
    }) => {
        await page.goto('/login');
        await page.emulateMedia({ reducedMotion: 'reduce' });
...
test.describe('prefers-reduced-motion: no-preference', () => {
    ...
    test('keeps the infinite animation when no preference is set', async ({
        page,
    }) => {
        await page.goto('/login');
        await page.emulateMedia({ reducedMotion: 'no-preference' });
