...
function matchesConsoleExpectation(
    text: string,
    expectation: ConsoleExpectation,
): boolean {
    return expectation instanceof RegExp
        ? expectation.test(text)
...
function matchesResponseExpectation(
    ...
    const urlMatches =
        expectation.url instanceof RegExp
            ? expectation.url.test(url)
            : url.includes(expectation.url);
