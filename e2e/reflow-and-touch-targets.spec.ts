...
test.describe('reflow at ~200% zoom (640x384), authenticated', () => {
  test.use({ storageState: STORAGE_STATE, viewport: { width: 640, height: 384 } })
...
test.describe('reflow at ~400% zoom (320x640), authenticated', () => {
  test.use({ storageState: STORAGE_STATE, viewport: { width: 320, height: 640 } })
...
test.describe('reflow at ~400% zoom (320x640) with a scaled root font-size', () => {
  ...
  test('/agenda has no page-level horizontal scroll at 24px root font-size', async ({ page }) => {
    await page.goto('/agenda')
    await page.waitForLoadState('networkidle')
    await page.evaluate(() => {
...
test.describe('reflow at ~400% zoom (320x640) with a scaled root font-size, public', () => {
  ...
  test('/login has no page-level horizontal scroll at 24px root font-size', async ({ page }) => {
...
test.describe('touch targets on a coarse-pointer device', () => {
  test.use({ hasTouch: true, isMobile: true, expectedIssues: ANONYMOUS_SESSION_PROBE_ISSUES })
...
test.describe('the touch-target exemption stays narrow', () => {
  ...
  test('only inline text links on /login carry it', async ({ page }) => {
    ...
    const exemptElements = await page.evaluate(() =>
      Array.from(document.querySelectorAll('.touch-target-exempt')).map((element) => ({
    ...
    expect(exemptElements.some((element) => element.text === 'Registrate')).toBe(true)
  })
})
