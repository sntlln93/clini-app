...
5. **Invented shadcn APIs**: for every shadcn/registry component the diff touches, verify each prop, variant and sub-component actually exists — via `mcp__shadcn__view_items_in_registries` (pass `registries: ["@shadcn"]`; omitting it wrongly reports no registries configured) or the installed source under `apps/panel/src/components/ui/`. A prop, variant or sub-component that is not in the registry is a REQUIRED finding, even when tsc passes — loose or `any`-typed props and `className` passthrough let invented APIs typecheck. Common invented shapes: `<Button isLoading>` / `isPending` (compose `Spinner` + `data-icon` + `disabled` instead), variant strings the component does not define, and items rendered outside their required Group. This check covers registry components only — for the project's own `src/components/` and `src/features/`, read the file.
