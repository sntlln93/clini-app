...
4. **For UI bugs, check the component's real API before theorizing.** A prop that silently does nothing because it does not exist is a common root cause, and it typechecks — loose or `any`-typed props and `className` passthrough hide it. Confirm props/variants against the local file, `apps/panel/src/components/ui/<name>.tsx` — it carries this project's local customizations, so it (not the registry) is the source of truth here. Confirm composition with `mcp__shadcn__get_item_examples_from_registries` — **pass it `registries: ["@shadcn"]`**, since omitting it wrongly reports that no registries are configured. `view_items_in_registries` is discarded for this check: it resolves the wrong variant (e.g. reports `Dependencies: radix-ui` even though this project is pure base-ui) and returns no file contents anyway (only name, type, file count and dependencies); it must also **not** be given a `registries` argument, since it declares `additionalProperties: false` and rejects it. The MCP needs the `panel` container up; if it errors, check `docker compose ps` — never fall back to a host `npx`. For the project's own `src/components/` and `src/features/`, read the source file instead.
5. **For bugs that look like a stack-library API changed** (React 19, TanStack Router/Query, Tailwind 4, Laravel 13), verify the real API with the `context7` MCP before concluding — `resolve-library-id` then `query-docs`, one concept per call — rather than trusting trained knowledge that may predate the pinned version. An outdated or hallucinated API call is a common root cause that presents as unrelated behavior. If the MCP is unreachable, say so in the report instead of guessing.
