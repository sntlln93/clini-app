...
4. **For UI bugs, check the component's real API before theorizing.** A prop that silently does nothing because it does not exist is a common root cause, and it typechecks — loose or `any`-typed props and `className` passthrough hide it. Confirm against the registry with `mcp__shadcn__get_item_examples_from_registries` — **pass it `registries: ["@shadcn"]`**, since omitting it wrongly reports that no registries are configured. Prefer it over `view_items_in_registries`, which returns no file contents (only name, type, file count and dependencies) and which must **not** be given a `registries` argument: it declares `additionalProperties: false` and rejects it. The MCP needs the `panel` container up; if it errors, check `docker compose ps` — never fall back to a host `npx`. For the project's own `src/components/` and `src/features/`, read the source file instead.
