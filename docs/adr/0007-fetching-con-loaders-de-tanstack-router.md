...
- **El hook compartido `useRefreshPageData` (sobre `refreshPageData`, `src/lib/page-data.ts`) para mutations que no navegan.** Una vez que una lectura vive solo en el loader, no queda ningún `useQuery` observador montado en esa página, así que `queryClient.invalidateQueries` con su comportamiento por defecto no dispara ningún refetch — y aunque lo disparara, `queryClient.ensureQueryData` (lo que usa el loader) resuelve el dato cacheado sin revisar si está invalidado, así que un `router.invalidate()` posterior solo vuelve a correr el loader contra la misma caché stale. `refreshPageData` invalida con `refetchType: 'all'` (evita el filtro de observador activo) y espera esa invalidación antes de llamar a `router.invalidate()`, para que el loader se re-ejecute contra datos frescos. Por eso, toda mutation cuyo éxito deja al usuario en la misma página cuyo dato vino de un loader debe llamar a `useRefreshPageData()` en su `onSuccess`, retornando su promesa. Si la mutation navega a otra ruta en su lugar, el loader de destino ya se ejecuta al llegar y no hace falta nada extra. Esta fue la causa de una regresión detectada en `/agenda` durante la migración: crear o reprogramar un turno sin salir de la página dejaba la agenda visible desactualizada hasta que el usuario navegaba y volvía.
