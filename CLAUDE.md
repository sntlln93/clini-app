...
- **The 8 button-like Base UI wrappers** (`Button`, `Checkbox`, `Switch`, `dropdown-menu.tsx`'s trigger/item variants, `SelectTrigger`/`SelectItem`, `dialog.tsx`/`sheet.tsx`'s trigger/close, `radio-group.tsx`'s item) require an explicit `nativeButton` when composed via `render` with a non-native element, since they render a native `<button>` by default and swapping it for e.g. `<Link>`/`<a>` without `nativeButton={false}` triggers a Base UI runtime warning (#54). Composing with `<button>` itself or with this project's own `Button`/`SidebarMenuButton` (which already default to a native `<button>`) is exempt from the check. Enforced by the local ESLint rule `local/require-native-button` (`apps/panel/eslint-rules/require-native-button.js`).
