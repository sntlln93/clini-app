...
- **A mutation that leaves the user on a page whose data came from a loader must call `router.invalidate()` in its `onSuccess`, alongside the existing `invalidateQueries`.** Once a read lives only in the loader, no `useQuery` observer remains mounted for it — `invalidateQueries` alone marks the query stale but has nothing to refetch, and the router does not re-run an already-mounted match's loader on its own. Keep `invalidateQueries` too, for any other observer outside the pattern. A mutation that navigates away instead needs nothing extra: the destination loader already runs on arrival. See [ADR 0007](docs/adr/0007-fetching-con-loaders-de-tanstack-router.md).
...
- **MCP server** `shadcn` (`.mcp.json`, project scope, versioned): the live registry — real props, files and usage examples. It runs as `docker compose exec -T --workdir /workspace/apps/panel panel npx -y shadcn@latest mcp`; the workdir is what lets it resolve `apps/panel/components.json`. **It requires the `panel` container to be up** (`docker compose up -d`) — with the container down every tool call fails, and the fix is to start it, never to fall back to a host `npx`.
  ...
  - **`view_items_in_registries` does not return file contents**, despite its description promising them — it returns only name, type, file count and dependencies. For real markup and prop usage, use `get_item_examples_from_registries`.
