...
- **MCP server** `shadcn` (`.mcp.json`, project scope, versioned): the live registry — real props, files and usage examples. It runs as `docker compose exec -T --workdir /workspace/apps/panel panel npx -y shadcn@latest mcp`; the workdir is what lets it resolve `apps/panel/components.json`. **It requires the `panel` container to be up** (`docker compose up -d`) — with the container down every tool call fails, and the fix is to start it, never to fall back to a host `npx`.
  - **Always pass `registries: ["@shadcn"]` explicitly.** Omitting it returns *"No registries are configured"* even though `get_project_registries` reports `@shadcn` as configured. That is a CLI quirk, not a misconfiguration, and not a reason to declare the MCP broken.
  - Available tools: `get_project_registries`, `list_items_in_registries`, `search_items_in_registries`, `view_items_in_registries`, `get_item_examples_from_registries`, `get_add_command_for_items`, `get_audit_checklist`. They are declared one by one in the `tools:` frontmatter of the `contractor` and `judge` agents — a subagent's `tools:` is an explicit allowlist, so an `mcp__shadcn__*` wildcard is not relied on there; adding a tool means adding its full name.
