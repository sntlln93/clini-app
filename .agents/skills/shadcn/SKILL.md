...
allowed-tools: Bash(docker compose exec --workdir /workspace/apps/panel panel npx shadcn@latest *)
---
...
> **This repo — always run through the `panel` container, never bare on the host.** `apps/panel`'s Node deps live inside the `panel` service (`apps/panel/compose.yaml`), not on the host — CLAUDE.md and `run-forensics.sh` are explicit that a local Node breaks version-sensitive tooling here. Every command below that starts with `npx shadcn@latest` must instead be run as, from the repo root, with the stack up (`docker compose up -d`):
>
> ```bash
> docker compose exec --workdir /workspace/apps/panel panel npx shadcn@latest <args>
> ```
>
> e.g. `npx shadcn@latest add button` → `docker compose exec --workdir /workspace/apps/panel panel npx shadcn@latest add button`. This project uses npm (not pnpm/bun) — ignore the `packageManager`-based runner selection below, it's already resolved.

## Current Project Context

```json
!`docker compose exec --workdir /workspace/apps/panel panel npx shadcn@latest info --json`
```
