...
The 5 subagents also have a Codex-native counterpart at `.codex/agents/<name>.toml` (Codex loads project subagents as TOML, one file per agent, from `.codex/agents/`, structurally incompatible with Claude Code's markdown-plus-YAML-frontmatter format — there is no symlink between them). **This is intentional duplication, not generated from a single source**: `.agents/agents/<name>.md`'s frontmatter (`name`/`description`) and full instruction body map onto the TOML's `name`/`description`/`developer_instructions` (a literal multiline string), `sandbox_mode` reflects the agent's real write access (`read-only` for `coroner`/`judge`, `workspace-write` for `contractor`/`stenographer`/`detective`), and `mcp_servers` mirrors whichever of `.mcp.json`'s servers appear in that agent's `tools:` list. When a subagent's `.md` changes, update its `.toml` counterpart by hand in the same commit — nothing enforces the two staying in sync.

**`developer_instructions` is verbatim for 4 of the 5 — `detective.toml` is the deliberate exception.** `coroner`/`judge`/`contractor`/`stenographer` copy their `.md` body unchanged (content-identical, checked byte-for-byte at authoring time), because their instructions don't depend on Claude Code's own runtime mechanics. `detective.md`'s "Delegation mechanics" and parts of "Budget", though, are written entirely around Claude Code-specific primitives (the `Agent` tool, `SendMessage`, task-notifications, a documented inability to poll/block on a subagent) that do not exist in Codex — and OpenAI's own docs describe Codex's subagent orchestration as model-mediated and internal, with no stable exposed API to translate those calls onto (third-party sources disagree with each other on the actual primitive names). Rather than invent Codex tool names to keep the section "translated" — the same failure mode every agent here is told to avoid ("never invent an API to keep moving") — `detective.toml`'s equivalent sections are rewritten in tool-agnostic, intent-level language: delegate and wait for the result before continuing, treat every continuation as a fresh invocation with no memory of the last one, keep state in file contracts, escalate rather than silently degrade if delegation is unavailable. That preserves the actual safety property (state lives in files, not subagent memory) without asserting a specific mechanism this project has not verified. If Codex's real orchestration primitives ever get confirmed via official docs, revisit this section specifically.
